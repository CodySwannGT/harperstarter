# your-project Operational Brain — LLM Wiki Contract

> Generated by the `lisa-wiki` plugin from `wiki/lisa-wiki.config.json`.
> kernelVersion: 2.62.1 · schemaVersion: 1.0.0 · rendered: 2026-05-25
...
## Purpose
The purpose of this wiki is the be the entire operational brain for this project.

## What this is
A git-native markdown knowledge base under `wiki/`, owned by **\<your-org>\** (set `org` in [wiki/lisa-wiki.config.json](../lisa-wiki.config.json) after templating), maintained by the
...
## Sources & connectors
Enabled connectors: git, documentation, memory, roles.
...
## Safety
- Source retention: **raw-ok**. Default sensitivity: **internal**.
...
## Git
PR per ingestion to main (auto-merge).

## README
Mode: **stub** (rich keeps the public README; stub points to `/onboard-me`; preserve leaves it).
