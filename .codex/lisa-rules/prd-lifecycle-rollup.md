...
This surfaces in real GitHub-backed PRD intake (e.g., a PRD issue generating a top-level Epic plus child Stories/Sub-tasks). The Epic should be a child/sub-issue of the PRD issue, and the PRD issue should eventually transition/close when the Epic and its descendants are complete — not before, and not twice.
